...
public interface UniversityProfileRepository extends JpaRepository<UniversityProfile, Integer> {
    @Query("select u from UniversityProfile u where u.universityName LIKE %?1% or u.city LIKE %?2% or u.city LIKE %?3% or u.city LIKE %?4%")
    List<UniversityProfile> findByUniversityNameAndCityAndStateAndZipIgnoreCase(String universityName, String city, String state, Integer zip);
}
